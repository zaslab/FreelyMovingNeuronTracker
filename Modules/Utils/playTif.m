...
%                                           written by Alex 10.12.14
    ...
    while (true)
        temp =  inputdlg({['Start index (1 to ' num2str(numOfFrames) '):'], 'end frame'},'',1,{'1',num2str(numOfFrames)});
    ...
    brightFactor = floor((1.0*MaxSampleValue(1))/double(max(max(frame)))*4/5);
    ...
    h = imshow(frame*brightFactor, 'InitialMagnification','fit');
    while true
        frame = imread(inputFile, frameNo,'Info', info);
        
        set(h, 'CData', (frame*brightFactor));
        title(['Frame:' num2str(frameNo) '   Brightness multiplier: ' num2str(brightFactor)]);
        [~, ~, button] = ginput(1);
